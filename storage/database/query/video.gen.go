...
func newVideo(db *gorm.DB, opts ...gen.DOOption) video {
	...
	_video.Topic = field.NewString(tableName, "topic")
...
type video struct {
	...
	PlayURL       field.String // 存文件名 然后灵活更换CDN域名
	CoverURL      field.String
	Title         field.String // Title 增加全文索引 以便于搜索 ngram全文索引支持中文的插件 默认分词2
	PublishTime   field.Time
	FavoriteCount field.Int64
	CommentCount  field.Int64  // 两个count是不是可以考虑删除 要的时候再去计算
	Topic         field.String // 视频的分类 前两个为固定字段 后面为tag隐式搜索
...
func (v *video) updateTableName(table string) *video {
	...
	v.Topic = field.NewString(table, "topic")
