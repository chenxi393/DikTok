...
func (s *CommentService) Add(ctx context.Context, req *pbcomment.AddRequest) (*pbbase.BaseResp, error) {
	zap.L().Sugar().Infof("[Add] req = %s", util.GetLogStr(req))
	resp, err := logic.Add(ctx, req)
	zap.L().Sugar().Infof("[Add] resp = %s, err = %s", util.GetLogStr(resp), err)
	return resp, err
}

func (s *CommentService) Delete(ctx context.Context, req *pbcomment.DeleteRequest) (*pbbase.BaseResp, error) {
	zap.L().Sugar().Infof("[Delete] req = %s", util.GetLogStr(req))
	resp, err := logic.Delete(ctx, req)
	zap.L().Sugar().Infof("[Delete] resp = %s, err = %s", util.GetLogStr(resp), err)
	return resp, err
}
...
func (s *CommentService) Count(ctx context.Context, req *pbcomment.CountReq) (*pbcomment.CountResp, error) {
	...
	zap.L().Sugar().Infof("[Count] resp = %s, err = %s", util.GetLogStr(resp), err)
	return resp, err
}
