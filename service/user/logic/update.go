...
func Update(ctx context.Context, req *pbuser.UpdateRequest) (*pbuser.UpdateResponse, error) {
	...
	case updatePassword:
		...
		encryptedPassword := util.BcryptHash(req.NewPassword)
...
func isUsernameOK(username string) error {
	...
	*/
	...
	//判断用户名存不存在
	_, err = storage.SelectUserByName(username)
