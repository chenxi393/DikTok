...
* 若数据库已经存在表的情况下，注释掉database里的init.go的migration()
* 否则MySQL主从复制有可能失败
...
- [ ] 主键自增消耗很快 考虑分布式ID生成 snowflake雪花算法？
- [ ] 用户名 评论 视频描述 消息敏感词过滤
...
* redis多表修改怎么保证原子性 一个出错怎么办 redis的事务特性 lua脚本？？
...
1. comment  done
   ...
   * 直接用zset存一个视频的所有评论（删除是数据库层面 可以删除）
2. video 
   * key: video_id value: json序列化后的结构体 （视频的结构体string）
   * key: video_id  value: count        （视频的count的计数对象 hash）
   * 分开计数信息和详细信息防止大key对哈希性能的影响？？
   * user_id value：用户发布publish的视频ID  （zset）
   * key: user_id value: video_id1 video_id2 （用户喜欢的视频 zset）
...
4. user
   * 用户的个人信息（这里基本不变）  string
   * 用户的个人信息的计数字段（这里变化比较大）  hash
...
* 排序集合使用Zset 无序使用set
* 注意video和user的信息会个别过期 需要一组user或者video信息
* 应当先去redis查出id 再查redis存在的，不存在的记录下来批量查数据库
...
* 当修改数据库时，为了确保数据一致性，需要`删除`掉redis的脏数据
