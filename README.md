### TODO
这样吧 先参照gin-mall商城快速实现一个可用的 然后根据需要增加中间件
再去看看别人怎么实现的 不然好难看懂别人的项目啊
...
1. go-zero进行微服务开发 使用数据库集群存储数据 实现软删除和分页分表 redis缓存热点数据（注意缓存命中率）---但是感觉只有真实业务才能确定什么是热点 `缓存穿透 缓存击穿 缓存雪崩`
